...
// to stdout
function rs2tabout1(&$rs,$addtitles=true)
{
	$fp = fopen('php://stdout','wb');
//	_adodb_export1($rs,",",' ',true,$addtitles);
	if ($fp) fclose($fp);
}

function _adodb_export1(&$rs,$sep,$sepreplace,$fp=false,$addtitles=true,$quote = '"',$escquote = '"',$replaceNewLine = ' ')
{
	if (!$rs) return '';
	//----------
	// CONSTANTS
	...
	if ($addtitles) {
		...
		while(list(,$o) = each($fieldTypes)) {
			...
			$v = strip_tags(str_replace("\n", $replaceNewLine, str_replace($NEWLINE,$replaceNewLine,str_replace($sep,$sepreplace,$v))));
			$elements[] = $v;
